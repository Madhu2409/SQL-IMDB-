...
-- To find the number of records in each table of Imdb

SELECT TABLE_NAME,TABLE_ROWS as Number_of_Records
...
/* Feedback: The maximum movie count released is during month of March. 
The movie released is least in the month of December.
The release of movies reduce in the month of February compared to January and then increases in the month of March
and then gradually decreases from March to July and then increases for the month August to October and then gradually 
decreases for Novemeber,December.*/
...
-- Type your code below:
Select count(id) as Total_count from movie 
...
select m.id,count(genre) as no_of_genre 
from genre g 
inner join movie m
on g.movie_id=m.id 
group by id )
...
select sum(total_votes) as total_votes_germany 
from ratings r 
inner join  movie m 
on r.movie_id=m.id 
where languages like "%German%"),
Italian_votes as(
select sum(total_votes) as total_votes_italy 
from ratings r 
inner join  movie m
 on r.movie_id=m.id
 where languages like "%Italian%"
)select * from Italian_votes,German_votes;
...
on m.id=r.movie_id
 ...
 top3_director as(
select genre, name as director_name,count(m.id) as movie_count,
...
inner join names n on d.name_id=n.id
 where avg_rating>8
 and g.genre in (select genre from genre_rank) 
 group by name )
 select * from top3_director where director_rank<=3;
...
on m.id=rm.movie_id
inner join names n
on rm.name_id=n.id where  category='actor' and country= 'india'  
group by name having count(m.id)>=5 limit 1;
...
m.id=g.movie_id group by genre order by count(id) desc limit 3),
top5_movies as(
...
select d.name_id,n.name,m.id,m.date_published,
...
avg_date as(
select name_id,avg(date_difference) as avg_inter_movie_days from date_diff
group by name_id),
final_result as (
select nmd.name_id as director_id,nmd.name as director_name,
count(m.id) as number_of_movies,round(avg_inter_movie_days,0) as avg_inter_movie_days,
...
from movie m inner join ratings r 
on m.id=r.movie_id 
inner join next_movie_date nmd
on m.id=nmd.id
inner join avg_date as ad on
nmd.name_id=ad.name_id
group by director_id)
select director_id,director_name,number_of_movies,avg_inter_movie_days,avg_rating,total_votes,
